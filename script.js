...
function spawnTrailCoordinates(clientX, clientY, count = 2) {
...
window.addEventListener('touchstart', (e) => {
    if (e.touches.length > 0) {
        ...
        spawnTrailCoordinates(lastTouchX, lastTouchY, 4);

        
        if (touchGenerationTimer) clearInterval(touchGenerationTimer);

        
        touchGenerationTimer = setInterval(() => {
            if (isTouching) {
                
                const jitterX = lastTouchX + (Math.random() * 16 - 8);
                const jitterY = lastTouchY + (Math.random() * 16 - 8);
                spawnTrailCoordinates(jitterX, jitterY, 1);
            }
        }, 30);
    }
}, { passive: true });
...
window.addEventListener('touchmove', (e) => {
    if (e.touches.length > 0) {
        ...
        lastTouchX = e.touches[0].clientX;
        lastTouchY = e.touches[0].clientY;
        spawnTrailCoordinates(lastTouchX, lastTouchY, 2);
    }
}, { passive: true });
...
window.addEventListener('touchend', () => {
    isTouching = false;
    if (touchGenerationTimer) {
        clearInterval(touchGenerationTimer);
        touchGenerationTimer = null;
    }
});
